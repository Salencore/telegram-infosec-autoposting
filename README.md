...
По умолчанию проект работает без OpenAI API: он берет тему дня из `content-plan-infosec.md` и публикует готовый статический текст. OpenAI можно подключить позже, если понадобится ежедневная генерация новых вариантов.
...
`OPENAI_API_KEY` для бесплатного режима не нужен.

...
- `state/images/` — автоматически сгенерированные PNG-карточки для постов.
...
Если `AUTOPUBLISH_INTERVAL_HOURS=12`, контейнер публикует следующий неопубликованный пост каждые 12 часов. В этом режиме он идет по календарю последовательно: день 1, день 2, день 3 и так далее, а не привязывается к сегодняшней дате.

К каждому посту автоматически создается PNG-карточка с заголовком и хуком. Бот отправляет картинку, затем текст поста.
...
5. Генерировать картинку-карточку.
6. Сразу публиковать картинку и готовый пост в Telegram-канал.

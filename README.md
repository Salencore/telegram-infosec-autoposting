...
По умолчанию проект работает без платных API: он берет тему дня из `content-plan-infosec.md` и публикует готовый статический текст. Gemini или OpenAI можно подключить, если понадобится ежедневная генерация новых вариантов.
...
`OPENAI_API_KEY` и `GEMINI_API_KEY` для статического режима не нужны.
